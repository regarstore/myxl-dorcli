...
def submit_otp(api_key: str, contact: str, code: str) -> dict:
...
def send_api_request(api_key: str, path: str, payload_dict: dict, id_token: str, method: str = "POST") -> dict:
    """Sends a signed and encrypted request to the MyXL API."""
    encrypted_payload = encryptsign_xdata(
        api_key=api_key, method=method, path=path, id_token=id_token, payload=payload_dict
    )
    ...
    try:
        ...
        decrypted_body = decrypt_xdata(api_key=api_key, encrypted_payload=resp.json())
...
def send_payment_request(api_key: str, payload_dict: dict, access_token: str, id_token: str, token_payment: str, ts_to_sign: int):
...
def purchase_package(api_key: str, tokens: dict, package_option_code: str) -> dict:
    ...
    purchase_result = send_payment_request(
        api_key, settlement_payload, tokens["access_token"], tokens["id_token"], token_payment, ts_to_sign
    )
